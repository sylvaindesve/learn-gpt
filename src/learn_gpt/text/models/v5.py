...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, embedding_dim: int, n_head: int, with_mask: bool = True):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.with_mask:
            # Masque du triangle haut droit de la matrice -> interdiction pour une
            # position de regarder une position future
...
# Un modèle v5 qui prédit toutes les positions
class StreamMultiHeadAttentionCharacterModel(nn.Module):
    def __init__(
        self,
        vocab_size: int,
        embedding_dim: int,
        ...
        self.attn = MultiHeadAttentionLayer(embedding_dim, n_head, with_mask)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Sortie de forme (n, block_size, vocab_size)
        #   -> pour chaque séquence et chaque position,
        #   le score de chaque token du vocabulaire
